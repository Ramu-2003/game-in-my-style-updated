...
const CSSLevelTemplate = ({ challenge, course, phase, levelNumber }) => {
  ...
  const handleCompile = async () => {
    ...
    setIsCompiling(true);
    setConsoleOutput([]);
    addConsoleMessage("🔄 Compiling CSS...", "info");
              ...
              <div className="bg-[#1e1e1e] rounded-lg shadow-md overflow-hidden border border-gray-700 flex flex-col" style={{ height: "500px" }}>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`px-4 py-1 rounded text-sm font-mono transition-colors ${
                        activeTab === 'css' ? 'bg-[#1e1e1e] text-blue-400' : 'text-gray-400 hover:text-gray-300'
                      }`}
                    >
                      styles.css
                    </button>
                    <button
                      ...
                      className={`px-4 py-1 rounded text-sm font-mono transition-colors ${
                        activeTab === 'html' ? 'bg-[#1e1e1e] text-orange-400' : 'text-gray-400 hover:text-gray-300'
                      }`}
                    >
                      index.html
                    </button>
                  </div>
                  <span className="text-gray-500 text-xs">{activeTab === 'css' ? 'CSS' : 'HTML'}</span>
                </div>
                ...
                <div className="flex-1 overflow-hidden">
                  {activeTab === 'css' ? (
                    ...
                    <div className="w-full h-full bg-[#1e1e1e] text-gray-100 font-mono text-sm p-4 overflow-auto">
                      <pre className="text-gray-400">{challenge.htmlCode}</pre>
                      <div className="mt-4 text-yellow-500 text-xs">
                        ⓘ HTML is read-only. Write CSS in styles.css tab.
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200 flex flex-col" style={{ height: "500px" }}>
                <div className="bg-green-50 px-4 py-2 border-b border-green-200">
                  <span className="text-green-700 text-sm font-semibold flex items-center gap-2">
                    <span className="text-green-600">✓</span> Expected Output
                  </span>
                </div>
                ...
                <div className="border-t border-gray-200">
                  <div
                    className="bg-gray-100 px-4 py-2 flex items-center justify-between cursor-pointer hover:bg-gray-200 transition-colors"
                    onClick={() => setIsExpectedOpen(!isExpectedOpen)}
                  >
                    <span className="text-gray-700 text-sm font-semibold flex items-center gap-2">
                      <span className="text-blue-600">▶</span> Your Output
                    </span>
                    {isExpectedOpen ? (
                  ...
                  {isExpectedOpen && (
                    <div className="bg-white overflow-auto p-4" style={{ height: "150px" }}>
                      {actualOutput ? (
                        <iframe
                          srcDoc={actualOutput}
                          title="output"
                        ...
                        <p className="text-gray-400 text-sm">Click COMPILE to see your output...</p>
                      )}
                    </div>
                  )}
                </div>
