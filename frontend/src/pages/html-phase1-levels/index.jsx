  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className="font-mono text-white/80 mt-1"
      ...
      <div className="flex flex-col items-center gap-3 px-5 py-5">
        {isLocked ? (
          ...
          <button
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.boxShadow = `0 6px 20px ${theme.accent}88`;
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.boxShadow = `0 4px 14px ${theme.accent}55`;
            }}
          >
            START
          </button>
...
const HTMLPhase1LevelsPage = () => {
  ...
  const fetchProgress = async () => {
    try {
      ...
      // Filter for html course, phase 1
      const phase1Completed = completed
        .filter((cl) => cl.course === "html" && cl.phase === 1)
        .map((cl) => cl.level);
  ...
  const isLevelUnlocked = (levelNum) => {
    if (levelNum === 1) return true; // Level 1 always unlocked
    return completedLevels.includes(levelNum - 1); // Level N unlocks when N-1 is done
  };
  ...
  const handleLevelStart = (levelNumber) => {
    navigate(`/levels/html/phase1/level${levelNumber}`);
  };
      ...
      <main className="flex-1 px-6 py-14 sm:px-10">
        ...
        <div className="max-w-7xl mx-auto mb-8">
          ...
          >
            ...
            <span className="font-techno font-bold text-sm uppercase tracking-wider">
              Back to Phases
            </span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {Array.from({ length: TOTAL_LEVELS }, (_, i) => i + 1).map((levelNum) => (
            <LevelCard
              ...
              levelData={htmlPhase1Levels[levelNum]}
            />
          ))}
        </div>
...
export default HTMLPhase1LevelsPage;
